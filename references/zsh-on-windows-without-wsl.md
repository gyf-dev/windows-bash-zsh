...
> 💡 **核心思路：** 本文介绍如何在不使用 WSL 的前提下，直接在 Git Bash for Windows 上安装 ZSH + Oh My ZSH，以获得比 WSL
> 更轻量、更快速的类 Unix 终端体验。
>
...
WSL（Windows Subsystem for Linux）虽然为 Windows 提供了类 Unix 环境，但其**性能和内存消耗较高**。本文将介绍一种替代方案——直接在
Git Bash for Windows 上安装 ZSH，无需启用 WSL。
...
从 MSYS2 软件包仓库下载最新的 MSYS2 ZSH 包。文件名格式为 `zsh-#.#-#-x86_64.pkg.tar.zst`。`.zst` 格式的压缩文件可以使用 *
*PeaZip** 解压。

### 安装

将解压后的 `etc` 和 `usr` 文件夹复制到 Git Bash 安装目录（通常为 `C:\Program Files\Git`），如果提示合并文件夹，选择**合并**
即可。
...
> ✅ **总结：** 通过以上步骤，你即可在 Windows 上拥有一个美观、高效的 ZSH 终端环境，无需启用 WSL。整个过程为：**Git Bash →
ZSH → Oh My Zsh! → Powerlevel10k 主题 → 插件 → 终端字体配置**，按顺序操作即可完成。
